...
const { Events, Users, Carts, sequelize } = require('../db.js');
...
const addEventToUserCart = async (req, res) => {
	...
	try {
		...
		if (!emptyCart.length) {
			...
			items = cart[0].dataValues.items;
			console.log('🐲🐲🐲 / file: Users.js / line 341 / items', items);

			await Carts.update(
				{ items: sequelize.fn('array_append', sequelize.col('items'), items) },
				{ where: { ID: id } }
			);
		}
		/* 		console.log('🐲🐲🐲 / file: Users.js / line 330 / cart', cart);
		 */ /*  */

		res.send('Event added to User model');
	} catch (error) {
		res.status(400).send(error.stack);
	}
};
